...
namespace Meso.Volunteer.Controllers {
    ...
    public class CommentApiController : ApiController {
        ...
        private readonly ICommentUpdateModelHandler _updateModelHandler;
        ...
        public CommentApiController(
            ...
            ICommentUpdateModelHandler updateModelHandler) {
        ...
        {
            if (inModel == null || inModel["CommentedItemId"] == null)
                return BadRequest();

            int commentedItemId = (int)inModel["CommentedItemId"];

            var commentsPart = _orchardServices.ContentManager.Get(commentedItemId).As<CommentsPart>();
        ...
        {
            if (!_orchardServices.Authorizer.Authorize(Orchard.Comments.Permissions.AddComment, T("Couldn't add comment")))
            ...
            if (ModelState.IsValid)
            {
                ...
                if (commentPart.Status == CommentStatus.Pending)
                {
                    ...
                    if (_orchardServices.Authorizer.Authorize(Orchard.Comments.Permissions.ManageComments))
                    {
        ...
        public IHttpActionResult update(JObject inModel)
        {
            ...
            if (!_orchardServices.Authorizer.Authorize(Orchard.Comments.Permissions.ManageComments, T("Couldn't edit comment")))
                return Unauthorized();// Ok(new ResultViewModel { Success = false, Code = HttpStatusCode.Unauthorized.ToString("d"), Message = "Couldn't edit comment" });
        ...
        public IHttpActionResult delete(JObject inModel)
        {
            ...
            if (!_orchardServices.Authorizer.Authorize(Orchard.Comments.Permissions.ManageComments, T("Couldn't delete comment")))
                return Unauthorized();// Ok(new ResultViewModel { Success = false, Code = HttpStatusCode.Unauthorized.ToString("d"), Message = "Couldn't delete comment" });
